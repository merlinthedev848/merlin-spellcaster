...
<div class="grid grid-2">
        ...
        <form method="post" action="?action=update">
            <div class="form-group">
                <label style="display: flex; align-items: center; gap: 8px; cursor: pointer; font-weight: 600; font-size: 13px; color: var(--stripe-dark);">
                    <input type="checkbox" name="warmup_active" value="1" <?= $warmupActive ? 'checked' : '' ?> style="cursor: pointer; accent-color: var(--stripe-blurple);">
            ...
            <div class="form-group">
                ...
                <span style="font-size: 11px; color: var(--stripe-dark-slate); margin-top: 4px; display: block;">Select a list containing seed email addresses you own (e.g. Gmail, Yahoo, Outlook accounts) to receive test emails.</span>
            </div>
    ...
    <div class="card" style="display: flex; flex-direction: column; gap: 20px;">
        ...
        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 16px; background-color: #fafbfc; border: 1px solid var(--stripe-border); padding: 16px; border-radius: 8px;">
            <div>
                <span style="font-size: 11px; text-transform: uppercase; color: var(--stripe-dark-slate); font-weight: 600; display: block; margin-bottom: 4px;">Engine Status</span>
                <?php if ($warmupActive): ?>
            ...
            <div>
                <span style="font-size: 11px; text-transform: uppercase; color: var(--stripe-dark-slate); font-weight: 600; display: block; margin-bottom: 4px;">Current Day</span>
                <span style="font-size: 14px; font-weight: 700; color: var(--stripe-dark);"><?= $day > 0 ? "Day {$day} of 30" : "—" ?></span>
            </div>
            <div>
                <span style="font-size: 11px; text-transform: uppercase; color: var(--stripe-dark-slate); font-weight: 600; display: block; margin-bottom: 4px;">Daily Send Quota</span>
                <span style="font-size: 14px; font-weight: 700; color: var(--stripe-dark);"><?= $quota > 0 ? "{$quota} emails/day" : "—" ?></span>
            </div>
        </div>

        <div>
            <h4 style="font-size: 12px; font-weight: 700; text-transform: uppercase; color: var(--stripe-dark); margin: 0 0 6px;">Daily Automation Cron Trigger</h4>
        ...
        <div style="background-color: rgba(245, 158, 11, 0.04); border: 1px dashed rgba(245, 158, 11, 0.2); padding: 12px; border-radius: 6px; font-size: 12px; color: var(--stripe-dark-slate); line-height: 1.5;">
