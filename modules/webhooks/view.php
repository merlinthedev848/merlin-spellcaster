...
<div class="grid grid-2">
    ...
    <div class="card" style="display: flex; flex-direction: column; gap: 20px;">
        ...
        <div class="table-wrapper" style="margin-top: 16px; border: 1px solid var(--theme-border);">
            <table style="font-size: 13px;">
                ...
                <tbody>
                    <?php if (empty($endpoints)): ?>
                        <tr>
                            <td colspan="4" style="text-align: center; color: var(--theme-dark-slate); padding: 20px;">No outbound webhooks defined.</td>
                        </tr>
                    <?php else: ?>
                        <?php foreach ($endpoints as $ep): ?>
                            <tr>
                                <td style="font-weight: 600; color: var(--theme-dark);"><?= e($ep['name']) ?></td>
                                <td><span class="badge badge-active" style="font-size:10px; font-weight:600;"><?= e($ep['event_type']) ?></span></td>
                                <td style="font-family: monospace; color: var(--theme-dark-slate); font-size:11px; max-width:180px; overflow:hidden; text-overflow:ellipsis; white-space:nowrap;" title="<?= e($ep['url']) ?>"><?= e($ep['url']) ?></td>
                                <td>
    ...
    <div class="card" style="display: flex; flex-direction: column; gap: 20px;">
        ...
        <p style="font-size: 13px; color: var(--theme-dark-slate); line-height: 1.6;">
        ...
        <div style="background-color: #fafbfc; border: 1px solid var(--theme-border); border-radius: 8px; padding: 16px;">
            <span style="font-size: 11px; text-transform: uppercase; color: var(--theme-dark-slate); font-weight: 600; display: block; margin-bottom: 6px;">POST Webhook URL</span>
        ...
        <div>
            <h4 style="font-size: 12px; font-weight: 700; text-transform: uppercase; color: var(--theme-dark); margin: 0 0 10px;">Payload Fields</h4>
        ...
        <div>
            <h4 style="font-size: 12px; font-weight: 700; text-transform: uppercase; color: var(--theme-dark); margin: 0 0 8px;">Your Target Segment List IDs</h4>
            <div style="display: grid; grid-template-columns: repeat(auto-fill, minmax(180px, 1fr)); gap: 8px;">
                <?php foreach ($lists as $l): ?>
                    <div style="background-color: #fafbfc; border: 1px solid var(--theme-border); padding: 8px 12px; border-radius: 6px; display: flex; justify-content: space-between; align-items: center;">
                        <span style="font-size: 12px; font-weight: 600; color: var(--theme-dark); overflow:hidden; text-overflow:ellipsis; white-space:nowrap; max-width:110px;" title="<?= e($l['name']) ?>"><?= e($l['name']) ?></span>
                        <span style="font-size: 11px; font-family: monospace; font-weight: 700; color: var(--theme-blurple);">ID: <?= $l['id'] ?></span>
                    </div>
                <?php endforeach; ?>
            </div>
        </div>
