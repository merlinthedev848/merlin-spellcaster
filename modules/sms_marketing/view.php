        ...
        <form method="post" action="?action=send">
            ...
            <div class="form-group" style="margin-bottom: 24px;">
                <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 8px;">
                    ...
                    <span id="char_counter" style="font-size: 11px; font-weight: 600; color: var(--stripe-dark-slate);">0 / 160 chars</span>
                </div>
                ...
                <p style="font-size: 11px; color: var(--stripe-dark-slate); margin-top: 4px;">Standard SMS limits are 160 characters. Submitting longer messages will split them automatically.</p>
            </div>
    ...
    <div class="card" style="padding: 24px;">
        ...
        <form method="post" action="?action=settings">
            ...
            <div class="form-group" style="margin-bottom: 24px;">
                ...
                <p style="font-size: 11px; color: var(--stripe-dark-slate); margin-top: 4px;">Use E.164 phone number formatting or a pre-registered alphanumeric Sender ID.</p>
            </div>
    ...
    <div class="table-wrapper">
        <table>
            ...
            <tbody>
                <?php if (empty($logs)): ?>
                    <tr>
                        <td colspan="4" style="text-align: center; color: var(--stripe-dark-slate); padding: 40px;">No SMS logs dispatched yet. Send a campaign above to test.</td>
                    </tr>
                <?php else: ?>
                    <?php foreach ($logs as $l): ?>
                        <tr>
                            <td style="font-weight: 600; color: var(--stripe-dark);"><?= e($l['phone']) ?></td>
                            <td style="color: var(--stripe-dark-slate); max-width: 500px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;"><?= e($l['message']) ?></td>
                            <td>
                                <span class="badge badge-active" style="font-size: 10px; font-weight: 700; text-transform: uppercase;">
                                    <?= e($l['status']) ?>
                                </span>
                            </td>
                            <td style="color: var(--stripe-dark-slate);"><?= date('M j, Y H:i:s', strtotime($l['created_at'])) ?></td>
                        </tr>
                    <?php endforeach; ?>
                <?php endif; ?>
            </tbody>
...
<script>
    function updateCharCount(textarea) {
        ...
        if (chars > 160) {
            ...
            counter.style.color = "var(--stripe-dark-slate)";
        }
    }
</script>
